...
class Player(pygame.sprite.Sprite):
	...
	def animate(self, weaponAngle, dx, dy):
		...
		if self.direction['left'] and not self.primarySelected or self.direction['left'] and not self.secondarySelected:
				self.image = pygame.transform.flip(self.image, True, False)

		if self.direction['right'] and not self.primarySelected or self.direction['right'] and not self.secondarySelected:
				self.image = self.originalImage
			
		# mouse controlled animation
		if weaponAngle < 270:
				if self.primarySelected or self.secondarySelected:
						self.image = pygame.transform.flip(self.image, True, False)
						self.direction['left'], self.direction['right'] = True, False
	...
	def update(self, dt, dx, dy, weaponAngle):
			self.airTimer += 1
			print(self.primarySelected)
			print(self.secondarySelected)
			print(self.selected)
			self.inventory.update(weaponAngle,
														self.selected), self.cooldown(), self.animate(
																weaponAngle, dx,
																dy), self.movement(dt), self.collision(dt)
